...
int q1(char data[])
{
  ...
  int diasMes[12] = {31,28 + bissexto,31, 30, 31, 30, 31, 31, 30, 31, 30, 31};
    if(dataQuebrada.iDia <1 || dataQuebrada.iDia > diasMes[dataQuebrada.iMes - 1]){
      return 0;
    }
      return 1;
...
DiasMesesAnos q2(char datainicial[], char datafinal[]){
  ...
  DataQuebrada dataFinal = quebraData(datafinal);
    if(dataFinal.iAno < dataInicial.iAno || dataFinal.iAno == dataInicial.iAno && dataFinal.iMes < dataInicial.iMes || dataFinal.iAno == dataInicial.iAno && dataFinal.iMes == dataInicial.iMes && dataFinal.iDia < dataInicial.iDia){
      dma.retorno = 4;
      return dma;
      }
      ...
      if (qtdDias < 0) {
          ...
          qtdDias += diasMes[dataInicial.iMes - 1];
      }
...
int ehCaractereAcentuado(unsigned char *strTexto, int posicao) {
  if (strTexto[posicao] == 0xC3 && (
    strTexto[posicao + 1] == 0xA1 || // á
